...
void test_list()
{
	{ //constructor definition check
		...
		LIST_T<int, 30, fixed::_impl::basic_heap_allocator> list_on_heap;
		l = list_on_heap;
		fixed::_impl::empty_source alloc_source_inst;
		LIST_T<int, 20, Alloc_pattern> l_with_alloc_source(alloc_source_inst);
		LIST_T<int, 20, Alloc_pattern> l_diff_with_alloc_source(list_on_heap, alloc_source_inst);
		l = l_with_alloc_source;
		l = expected;
		CHECK(std::equal(l.begin(), l.end(), expected.begin(), expected.end()));
		list_on_heap = expected;

		LIST_T<int, 20, Alloc_pattern> l_diff_with_alloc_source_not_empty(list_on_heap, alloc_source_inst);
		...
		LIST_T<int, 20, Alloc_pattern> l_move_alloc(std::move(l_move), alloc_source_inst);
		...
		LIST_T<int, 20, Alloc_pattern> l_with_values_alloc(5, 2, alloc_source_inst);
		CHECK(l_with_values_alloc.size() == 5);
		CHECK(std::all_of(l_with_values.begin(), l_with_values.end(), [](const auto& v) {return v == 2; }));

		LIST_T<int, 20, Alloc_pattern> l_with_iterator(expected.begin(), expected.end());
		...
		LIST_T<int, 20, Alloc_pattern> l_with_iterator_alloc(expected.begin(), expected.end(), alloc_source_inst);
...
void test_modifiers()
{
	...
	{
		LIST_T<int, 30, Alloc_pattern> l = { 1, 2, 3, 4, 5 };
	...
	{
		LIST_T<test, 30, Alloc_pattern> l;
		const char* exp = "test";
		l.emplace_back("test");
	...
	{
		LIST_T<int, 5, Alloc_pattern> l{ 0, 1, 2, 3, 4 };
		auto& val = *(l.begin() + 4);
		CHECK(val == 4);
