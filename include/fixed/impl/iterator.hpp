...
namespace fixed
{
	namespace _impl 
	{
		...
		{
		public:
			typedef typename std::remove_const<typename std::remove_pointer<T>::type>::type value_type;
			typedef typename value_type* pointer;
			typedef typename value_type& reference;
			typedef std::ptrdiff_t difference_type;
			...
			pointer_iterator(const T* val) : _value(const_cast<T*>(val)) {}
			...
			template <typename = std::enable_if<std::is_const<typename std::remove_pointer<T>::type>::value>>
			pointer_iterator(const pointer_iterator<value_type>& orig)
				: _value(orig._value)
			{}


			operator reference() { return _value; }
		...
		public:
			...
			const_pointer_iterator(const T* val) : _value(const_cast<T*>(val)) {}
