...
namespace fixed
{
	namespace _impl
	{
		...
		private:
			...
			Alloc_pattern<node, SIZE> _bucket_data; //compact index to data, handle memory recycling
			Alloc_pattern<value_type, SIZE> _data; //sparse data holder
			size_type _size = 0; //total size of initialized object & index to the next object 
